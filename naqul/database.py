...
class Database:
    ...
    def __init__(self):
        self.tables = []
        self.wordsys_object = None

    def set_wordsys(self, wordsys):
        self.wordsys_object = wordsys
    ...
    def create_table(self, table_string):
        ...
        for line in lines:
            if 'TABLE' in line:
                ...
                if self.wordsys_object is not None:
                    table.equivalences = self.wordsys_object.get_synonyms_of_a_word(table.name)
                ...
                if column_name is not None:
                    ...
                    if self.wordsys_object is not None:
                        equivalences = self.wordsys_object.get_synonyms_of_a_word(column_name.group(1))
                    else:
                        equivalences = []
        ...
        for table in self.tables:
            ...
            for column in table.columns:
                if column.is_primary():
                    ...
                    print("| # %31s           |" % (Color.BLUE + column.name + ' (' + column.type + ')' + Color.END))
                else:
